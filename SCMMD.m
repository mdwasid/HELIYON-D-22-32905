%% Single Criteria approach based on Modified Mahalanobis Distance (SCMMD)
    ...
    for actuser=1 :db_size(1)
        ...
        for g = 1:db_size(1)
            if g ~= actuser
                ...
                %% Caclulating the similarity among different users using modified mahalanabosis distance
                ...
                for d = 1: train_size(1,actuser)
                    if  userxitem_db(g,train_index(d)) ~=0
                        mat(:,1)=userxitem_db(actuser,:);       %mahalanabosis distance implementation
                        mat(:,2)=userxitem_db(g,:);
                        cov_mat=cov(mat);
                        sigma1=sqrt(cov_mat(1,1));
                        sigma2=sqrt(cov_mat(2,2));
                        raw12=(cov_mat(1,2)/(sigma1*sigma2));
                        temp=(train(d)- actuser_mean)/sigma1;
                        maha_dis(d)=sqrt(temp^2 + ((((userxitem_db(g,train_index(d))- User_mean(g))/sigma2)-(raw12*temp))*(1/(sqrt(1-raw12^2))))^2);
                        comm=comm+1;
                    end
                end
                if comm ~=0
                    sim(g,1)=sum(maha_dis)/comm; %similarity between actuser and user g
                else
                    sim(g,1)=999;
                end
            else
            ...
            for pr=1:ni            % movies to predict by neighbourhood set
                ...
                for n=1:size(neighbour,1)          % nighbours
                    if  userxitem_db(neighbour(n,1),test_index(1,pr))~=0 % && userxitem_db(actuser,test_index(1,pr)) ~=0  #### already non-zero coz it's in test_index
                        ...
                        norm_k = norm_k + abs(sim(neighbour(n,1),1));
                        count = count+1;
                    end
                end
