%% Normalized rating count and common rating weight similarity (NRC-CRS)
...
%% Calculating multi-criteria rating count
    ...
    for g = 1:db_size(1)
        if g ~= actuser
            ...
            for p=1:5:20
                sim1=0;
                for k=p:p+4
                    sim1 =sim1+(freq_count(actuser,k)-freq_count(g,k))^2;
                end
                sim2=sim2+sqrt(sim1);
            end
            mcfc(g,actuser)=1/(1+(sim2/4));
        else
            mcfc(g,actuser)=99;
        end
    end
...
for train_test=.60:.10:.90
    ...
    actuser_mae=zeros(db_size(1),1); actuser_rmse=zeros(db_size(1),1); actuser_corpred=zeros(db_size(1),1); train_size= zeros(1,db_size(1));
    ...
    for actuser=1 :db_size(1) %number of users
        ...
        for g = 1:db_size(1)
            if g ~= actuser
                ...
                actuser_mean = User_mean(actuser);  % udate the mean of active user only
                ...
                denoR = (1/(1+exp(-(comm_rat/4)))); %Common rating weight similarity
        ...
        for top_k=10:10:70
            ...
            for pr=1:ni            % movies to predict by neighbourhood set
                ...
                if count ~=0 || (right ~=0 && norm_k ~= 0)     %avoid devide-by-zero-problem        
            ...
            %% Precision,Recall, F-measure and accuracy measures
