%% Single Criteria approach based on Pearson Correlation (SCPC)
    ...
    for actuser=1 :db_size(1) %484 number of users
        ...
        for g = 1:db_size(1)
            if g ~= actuser
                ...
                %% Caclulating the similarity among different users using pearson correlation
                check =0 ; denoL=0; denoR=0;
                for d = 1: train_size(1,actuser)
                    if  userxitem_db(g,train_index(d)) ~=0
                        sim1(g) = sim1(g) + ((train(d)- actuser_mean)*(userxitem_db(g,train_index(d))- User_mean(g)));
                        denoL= denoL + (train(d)- actuser_mean)^2;
                        denoR = denoR+ (userxitem_db(g,train_index(d))- User_mean(g))^2;
                        check=check+1;
                    end
                end
                if check > 0        %true if atleast one common item found between U and V
                    sim2 = sqrt(denoL)*sqrt(denoR);
                    if sim2 ~=0
